...
class FrontendProfilesProfile
{
	...
	{
	/*	$item = array();
		$item['profileid'] = $this->getId();
		$item['message'] = $message;
		$item['created_on'] = BackendModel::getUTCDate();
		
		FrontendProfilesModel::addLog($item);*/
	}
	...
	/**
	 * Get languages of a profile
	 *
	 * @return string 
	 */
	public function getLanguages()
	{
		$languages = (array) FrontendModel::getContainer()->get('database')->getColumn(
			'SELECT l.name
			 FROM languages AS l
			 INNER JOIN profiles_languages AS pl ON l.id = pl.languageid
			 WHERE pl.profileid = ? AND l.language = ?
	 ...
	 * @param string	$message
	 */
	public function notify($module, $message)
	{
		$item = array();
		$item['profileid'] = $this->getId();
		$item['module'] = $module;
		...
		// save id
		$notificationId = FrontendNotificationsModel::insert($item);
	...
	public function toArray()
	{
		...
		$return['gravatar_id'] = $this->getGravatarId();
