 ...
 */
class FrontendForumEditPost extends FrontendBaseBlock
{

	/**
	 * Post
	 *
	 * @var FrontendForumPost
	 */
	private $item;
	...
	public function execute()
	{
		...
		if(FrontendProfilesAuthentication::isLoggedIn())
		{
			...
			if($id == 0 || !BackendForumModel::existsPost($id))
			{
	...
	{
		// get data
		$this->item = new FrontendForumPost($id);
	...
	{
		// delete url
		$this->tpl->assign('deleteUrl', SITE_URL . FrontendNavigation::getURLForBlock('forum', 'delete_post') . '?id=' . $this->item->getId());
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				// redirect
				$this->redirect(SITE_URL . FrontendNavigation::getURLForBlock('forum', 'detail') . '/' . $this->item->getTopic()->getUrl() . '#post-' . $this->item->getId());
			}
		}
	}
}
